...
releases = {r.title: r for r in repo.get_releases() if len(r.title)>1 and r.title.startswith('v')}

full_releases = {k: v for k, v in releases.items() if re.match('^v[\.0-9]*$', k)}
